...
BATCH_SIZE = 32
EPOCHS = 20  # Increased for fine-tuning depth
LEARNING_RATE = 0.0001  # Lower LR for fine-tuning to preserve pre-trained knowledge

def train():
    print("--- STARTING FINAL PROFESSIONAL TRAINING (MobileNetV2) ---")
    
    # Advanced Data Augmentation for best accuracy
    data_transforms = {
    ...
    print("Loading datasets with torchvision...")
    image_datasets = {
    ...
    # Use the shared architecture with fine-tuning enabled
    model = ai_model.get_model(len(class_names), fine_tune=True).to(device)
    
    criterion = nn.CrossEntropyLoss()
    # Optimize the WHOLE model now
